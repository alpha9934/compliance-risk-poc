...
def filter_output(data: dict, permitted_fields: set[str] | None = None) -> dict:
    ...
    Empty set means no fields permitted — returns {}.
    None means apply the default blocked-fields list instead.
    """
    if permitted_fields is not None:      # explicit set provided — whitelist mode
